...
# reviewed Markdown, and exactly the eight active model/metadata files.
COPY backend/ ./backend/
COPY README.md ./
COPY docs/ ./docs/
COPY scripts/train_ps3.py scripts/check_deployment_models.py ./scripts/
COPY deploy/oracle/model-reference.json ./deploy/oracle/model-reference.json
